...
function System() {
  ...
  useEffect(() => {
    ...
    socket.on("factcheck", (newTruth) => {
      setFactCheck(factCheck => (newTruth as string));
    })
    ...
    socket.on("server_status", (status) => {
      ...
      setRunSpeakFlag(status["speak_flag"] ?? false)
      setResetDialogFlag(status["reset_dialog_flag"] ?? false)
      setLLMModel(status["llm_model"] ?? "")
      setAvailableModels(status["available_models"] ?? [])
      setPromptPresets(status["prompt_presets"] ?? [])
      // setPromptSetup(promptPresets[0].prompt)
