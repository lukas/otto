...
prompt_setup = """You are Oren the Octopus a friendly chatbot. You are having a conversation with a human child."""
...
def number_people_talking(lookback_duration=1.0):
    newest_timestamp = 0
    speakers = set()
    p = subprocess.Popen(["tail", "-10", "-r", diart_filename], stdout=subprocess.PIPE)
    diart_lines = ""
...
def talk(user_prompt):
    ...
    socket_io.emit("old_prompts", {"old_prompts": old_prompts, "old_responses": old_responses})
...
def listen():
    print("In listen")
    # global tts_log_data
    p = subprocess.Popen(["tail", "-1", transcribe_filename], stdout=subprocess.PIPE)
    lines = []
    while 1:
...
def update():
    p = subprocess.Popen(["tail", "-10", transcribe_filename], stdout=subprocess.PIPE)
    tts = p.communicate()[0]
